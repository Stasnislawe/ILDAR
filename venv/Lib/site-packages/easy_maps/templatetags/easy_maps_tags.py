...
class EasyMapTag(InclusionTag):
    ...
    {% easy_maps <address> [<width> <height>] [<zoom>] [using <template_name>] %}
    ...
    def render_tag(self, context, **kwargs):
        ...
        if "address" in params and (len(params) == 2 or len(params) > 4):
            raise template.TemplateSyntaxError(
                "easy_maps tag has the following syntax: "
                "{% easy_maps <address> [<width> <height>] [zoom] [using <template_name>] %}"
            )

        if settings.EASY_MAPS_GOOGLE_KEY is None:
            raise ImproperlyConfigured(
                "easy_maps tag requires EASY_MAPS_GOOGLE_KEY to be set in global settings "
